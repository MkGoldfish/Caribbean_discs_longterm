...
### Incubation -----------------------------------------------------------------
Phyla <- tt.inc  %>%  select(Location, Habitat, Polymer, Isotope, Polymer_Isotope, Backbone, Treatment, Phylum, Phylum_rel_abund_Sample)%>% 
  distinct() 
...
### Wild plastic-----------------------------------------------------------------
Phyla <- tt.wild  %>%  select(Location, Habitat, Description,  Phylum, Phylum_rel_abund_Sample)%>% 
  distinct() 
Phyla_filtAb <- Phyla %>% mutate(Phylum = ifelse(Phylum_rel_abund_Sample<0.02, "others<2.0%", Phylum))
...
### Incubation -----------------------------------------------------------------
Orders <- tt.inc  %>%  select(Location, Habitat, Polymer, Isotope, Polymer_Isotope, Backbone, Treatment, Order, Order_rel_abund_Sample)%>% 
  distinct() 
...
### Wild plastic-----------------------------------------------------------------
Orders <- tt.wild  %>%  select(Location, Habitat, Description, Order, Order_rel_abund_Sample)%>% 
  distinct() 
