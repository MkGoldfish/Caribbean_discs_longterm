...
## Import data ----------------------------------------------------------------------------
tt <- read.csv("../Processed-Data/NIOZ164_EUX_discs_Proks_tidy_data_decontamed_pruned_RA.csv", row.names = NULL)
head(tt)
tt$X <- NULL
...
gg_King <- ggplot(Kingdom, aes(x=interaction(Polymer_Isotope, Backbone), y= Kingdom_rel_abund_Sample, 
  ...
  facet_nested( Habitat ~ Location + Treatment, drop = T, 
                axes = 'margins',
...
### Wild plastic-----------------------------------------------------------------
Kingdom <- tt.filt  %>%  select(Location, Habitat,Description, Kingdom, Kingdom_rel_abund_Sample)%>% 
  distinct() 
...
### Incubation -----------------------------------------------------------------
Phyla <- tt.inc  %>% filter(Kingdom != "EUkaryota") %>% select(Location, Habitat, Polymer, Isotope, Polymer_Isotope, Backbone, Treatment, Phylum, Phylum_rel_abund_Sample)%>% 
  distinct() 
...
### Wild plastic-----------------------------------------------------------------
Phyla <- tt.wild %>% filter(Kingdom != "EUkaryota") %>%  select(Location, Habitat, Description,  Phylum, Phylum_rel_abund_Sample)%>% 
  distinct() 
Phyla_filtAb <- Phyla %>% mutate(Phylum = ifelse(Phylum_rel_abund_Sample<0.02, "others<2.0%", Phylum))
...
### Filters -----------------------------------------------------------------
Phyla <- tt.filt %>% filter(Kingdom != "EUkaryota") %>%  select(Location, Habitat, Description,  Phylum, Phylum_rel_abund_Sample)%>% 
  distinct() 
Phyla_filtAb <- Phyla %>% mutate(Phylum = ifelse(Phylum_rel_abund_Sample<0.02, "others<2.0%", Phylum))
...
### Incubation -----------------------------------------------------------------
Orders <- tt.inc  %>% filter(Kingdom != "EUkaryota") %>%  select(Location, Habitat, Polymer, Isotope, Polymer_Isotope, Backbone, Treatment, Order, Order_rel_abund_Sample)%>% 
  distinct() 
...
### Wild plastic-----------------------------------------------------------------
Orders <- tt.wild %>% filter(Kingdom != "EUkaryota") %>%  select(Location, Habitat, Description, Order, Order_rel_abund_Sample)%>% 
  distinct() 
