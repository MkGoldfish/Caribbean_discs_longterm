...
map <- map %>%
  add_column(Location_Habitat, .before ="Polymer") %>% 
  sample_data()
...
## Read abundance per sample plot
```{r abundance plot}
sample_sums <- sample_sums(physeq_object)
sample_sums_df <- data.frame(sample = map$Description, sum = sample_sums)
ggplot(sample_sums_df, aes(x = sample, y = sum)) +
